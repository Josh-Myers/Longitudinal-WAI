...
library(car)
library(glmmTMB)
# need to install old version of lsmeans (not emmeans) to make the hack work with glmmTMB
...
ear.canal.age = lmer(S_log ~ maternal.ethnicity + gender + ear + (1 | id.res/ear.id), ear.canal.df, REML = FALSE)
ear.canal.age.anova = anova(ear.canal.age, ear.canal.f_log)
ear.canal.age.anova = ear.canal.age.anova[2,6:8]

ear.canal.eth = lmer(S_log ~ age.group + gender + ear + (1 | id.res/ear.id), ear.canal.df, REML = FALSE)
ear.canal.eth.anova = anova(ear.canal.eth, ear.canal.f_log)
ear.canal.eth.anova = ear.canal.eth.anova[2,6:8]

ear.canal.ear = lmer(S_log ~ age.group * maternal.ethnicity + gender + (1 | id.res/ear.id), ear.canal.df, REML = FALSE)
ear.canal.ear.anova = anova(ear.canal.ear, ear.canal.f_log)
ear.canal.ear.anova = ear.canal.ear.anova[2,6:8]

ear.canal.sex = lmer(S_log ~ age.group * maternal.ethnicity + ear + (1 | id.res/ear.id), ear.canal.df, REML = FALSE)
ear.canal.sex.anova = anova(ear.canal.sex, ear.canal.f_log) 
ear.canal.sex.anova = ear.canal.sex.anova[2,6:8]

## and then interactions
# test age x ethnicity interaction
ear.canal.age.eth = lmer(S_log ~ maternal.ethnicity + age.group + gender + ear + (1 | id.res/ear.id), ear.canal.df, REML = FALSE)
ear.canal.age.eth.anova = anova(ear.canal.age.eth, ear.canal.f_log) 
ear.canal.age.eth.anova = ear.canal.age.eth.anova[2,6:8]

# make table
ear.canal.anova = rbind.data.frame(ear.canal.age.anova, ear.canal.eth.anova, ear.canal.ear.anova, ear.canal.sex.anova, ear.canal.age.eth.anova)
rownames(ear.canal.anova) = c("age", "ethnicity", "ear", "gender", "age:ethnicity")
ear.canal.anova = t(ear.canal.anova)
ear.canal.anova = as.data.frame(ear.canal.anova)
ear.canal.anova$stats = rownames(ear.canal.anova)
ear.canal.anova$measure = "Ear canal"
ear.canal.anova$model = "LMM log"
ear.canal.anova$Frequency = "NA"
ear.canal.anova$Frequency.age = "NA"
ear.canal.anova$Frequency.ethnicity = "NA"
ear.canal.anova$Frequency.ethnicity.age = "NA"
ear.canal.anova = ear.canal.anova[c(7, 8, 6, 9, 1:4, 5, 10:12)]

# post hoc analyses (plot 95% ci)
ear.canal.lsmean.age = lsmeans(ear.canal.f_log, specs = "age.group") 
...
# set <0 to >0 - glmmTMB needs to be above 0
abs.2.long$Absorbance_pos = abs.2.long$Absorbance
abs.2.long$Absorbance_pos[abs.2.long$Absorbance_pos <= 0] <- 0.0001
...
abs.f.freq = glmmTMB(Absorbance_pos ~ age.group * maternal.ethnicity + gender + ear + (1 | id.res/ear.id), 
                     abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.freq.anova = anova(abs.f.freq, abs.f.b.full)
abs.freq.anova = abs.freq.anova[2,6:8]

abs.f.age = glmmTMB(Absorbance_pos ~ Frequency * maternal.ethnicity + gender + ear + (1 | id.res/ear.id), 
                    abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.age.anova = anova(abs.f.age, abs.f.b.full)
abs.age.anova = abs.age.anova[2,6:8]

abs.f.eth = glmmTMB(Absorbance_pos ~ Frequency * age.group + gender + ear + (1 | id.res/ear.id), 
                    abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.eth.anova = anova(abs.f.eth, abs.f.b.full)
abs.eth.anova = abs.eth.anova[2,6:8]

abs.f.b.ear = glmmTMB(Absorbance_pos ~ Frequency * age.group * maternal.ethnicity + gender + (1 | id.res/ear.id), 
                      abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.ear.anova = anova(abs.f.b.ear, abs.f.b.full)
abs.ear.anova = abs.ear.anova[2,6:8]

abs.f.b.sex = glmmTMB(Absorbance_pos ~ Frequency * age.group * maternal.ethnicity + ear + (1 | id.res/ear.id), 
                      abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.sex.anova = anova(abs.f.b.sex, abs.f.b.full) 
abs.sex.anova = abs.sex.anova[2,6:8]

## and then interactions
# test age x ethnicity interaction
abs.f.b.age.eth = glmmTMB(Absorbance_pos ~ Frequency * (maternal.ethnicity + age.group) + 
                            gender + ear + (1 | id.res/ear.id), abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.age.eth.anova = anova(abs.f.b.age.eth, abs.f.b.full) 
abs.age.eth.anova = abs.age.eth.anova[2,6:8]

# test freq x age interaction
abs.f.b.freq.age = glmmTMB(Absorbance_pos ~ Frequency * maternal.ethnicity + age.group + age.group:maternal.ethnicity +
                             gender + ear + (1 | id.res/ear.id), abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.freq.age.anova = anova(abs.f.b.freq.age, abs.f.b.full) 
abs.freq.age.anova = abs.freq.age.anova[2,6:8]

# test frequency x ethnicity interaction
abs.f.b.freq.eth = glmmTMB(Absorbance_pos ~ Frequency * age.group + maternal.ethnicity + age.group:maternal.ethnicity +
                             gender + ear + (1 | id.res/ear.id), abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.freq.eth.anova = anova(abs.f.b.freq.eth, abs.f.b.full) 
abs.freq.eth.anova = abs.freq.eth.anova[2,6:8]

#Freq:maternal.ethnicity:age
abs.f.b.freq.eth.age = glmmTMB(Absorbance_pos ~ Frequency + maternal.ethnicity + age.group + 
                                 Frequency:maternal.ethnicity + 
                                 age.group:maternal.ethnicity + 
                                 age.group:Frequency +
                                 gender + ear + (1 | id.res/ear.id), abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.freq.age.eth.anova = anova(abs.f.b.freq.eth.age, abs.f.b.full) 
abs.freq.age.eth.anova = abs.freq.age.eth.anova[2,6:8]

# make table
abs.anova = rbind.data.frame(abs.freq.anova, abs.age.anova, abs.eth.anova, abs.ear.anova, abs.sex.anova, 
                             abs.age.eth.anova, abs.freq.age.anova, abs.freq.eth.anova, abs.freq.age.eth.anova)
rownames(abs.anova) = c("frequency", "age", "ethnicity", "ear", "gender", "age:ethnicity", "freq:age", "freq:ethnicity", "freq:age:ethnicity")
abs.anova = as.data.frame(t(abs.anova))
abs.anova$stats = rownames(abs.anova)
abs.anova$measure = "absorbance"
abs.anova$model = "beta glmm"
abs.anova = abs.anova[c(11, 12, 10, 1:9)] 

# set final model to reml = T
abs.f.final = update(abs.f.b.full, REML = TRUE) 
...
overdisp_fun(abs.f.final) # not signficant (this is good - like gof tests - significant is bad)
...
abs.lsmean.eth = lsmeans(abs.f.final, specs = "Frequency", by = c("age.group", "maternal.ethnicity"), type = "response")
...
# make table
mag.anova = rbind.data.frame(mag.freq.anova, mag.age.anova, mag.eth.anova, mag.ear.anova, mag.sex.anova, 
                             mag.age.eth.anova, mag.freq.age.anova, mag.freq.eth.anova, mag.freq.age.eth.anova)
rownames(mag.anova) = c("frequency", "age", "ethnicity", "ear", "gender", "age:ethnicity", "freq:age", "freq:ethnicity", "freq:age:ethnicity")
mag.anova = as.data.frame(t(mag.anova))
mag.anova$stats = rownames(mag.anova)
mag.anova$measure = "|Y|"
mag.anova$model = "LMM log"
mag.anova = mag.anova[c(11, 12, 10, 1:9)] 
...
# post hoc plots
mag.lsmean.age = lsmeans(mag.f.final, specs = "Frequency", by = "age.group", type = "response")
...
# plot ethnicity effect
mag.lsmean.eth = lsmeans(mag.f.final, specs = "Frequency", by = c("age.group", "maternal.ethnicity"), type = "response")
...
# make table
mag.norm.anova = rbind.data.frame(mag.norm.freq.anova, mag.norm.age.anova, mag.norm.eth.anova, mag.norm.ear.anova, mag.norm.sex.anova, 
                                  mag.norm.age.eth.anova, mag.norm.freq.age.anova, mag.norm.freq.eth.anova, mag.norm.freq.age.eth.anova)
rownames(mag.norm.anova) = c("frequency", "age", "ethnicity", "ear", "gender", "age:ethnicity", "freq:age", "freq:ethnicity", "freq:age:ethnicity")
mag.norm.anova = as.data.frame(t(mag.norm.anova))
mag.norm.anova$stats = rownames(mag.norm.anova)
mag.norm.anova$measure = "Normalized |Y|"
mag.norm.anova$model = "LMM log"
mag.norm.anova = mag.norm.anova[c(11, 12, 10, 1:9)] 
...
# post hoc plots
mag.norm.lsmean.age = lsmeans(mag.norm.f.final, specs = "Frequency", by = "age.group", type = "response")
...
# plot ethnicity effect
mag.norm.lsmean.eth = lsmeans(mag.norm.f.final, specs = "Frequency", by = c("age.group", "maternal.ethnicity"), type = "response")
...
# make table
g.norm.anova = rbind.data.frame(g.norm.freq.anova, g.norm.age.anova, g.norm.eth.anova, g.norm.ear.anova, g.norm.sex.anova, 
                                g.norm.age.eth.anova, g.norm.freq.age.anova, g.norm.freq.eth.anova, g.norm.freq.age.eth.anova)
rownames(g.norm.anova) = c("frequency", "age", "ethnicity", "ear", "gender", "age:ethnicity", "freq:age", "freq:ethnicity", "freq:age:ethnicity")
g.norm.anova = as.data.frame(t(g.norm.anova))
g.norm.anova$stats = rownames(g.norm.anova)
g.norm.anova$measure = "Normalized G"
g.norm.anova$model = "LMM Box-Cox C+1"
g.norm.anova = g.norm.anova[c(11, 12, 10, 1:9)] 
...
# post hoc plots
g.norm.lsmean.age = lsmeans(g.norm.f.final, specs = "Frequency", by = "age.group", type = "response")
...
# plot ethnicity effect
g.norm.lsmean.eth = lsmeans(g.norm.f.final, specs = "Frequency", by = c("age.group", "maternal.ethnicity"), type = "response")
...
# make table
b.norm.anova = rbind.data.frame(b.norm.freq.anova, b.norm.age.anova, b.norm.eth.anova, b.norm.ear.anova, b.norm.sex.anova, 
                                b.norm.age.eth.anova, b.norm.freq.age.anova, b.norm.freq.eth.anova, b.norm.freq.age.eth.anova)
rownames(b.norm.anova) = c("frequency", "age", "ethnicity", "ear", "gender", "age:ethnicity", "freq:age", "freq:ethnicity", "freq:age:ethnicity")
b.norm.anova = as.data.frame(t(b.norm.anova))
b.norm.anova$stats = rownames(b.norm.anova)
b.norm.anova$measure = "Normalized B"
b.norm.anova$model = "LMM log C+2"
b.norm.anova = b.norm.anova[c(11, 12, 10, 1:9)] 
names(ear.canal.anova) = c("measure", "model", "stats", "frequency", "age", "ethnicity", "ear", "gender", "age:ethnicity", "freq:age", "freq:ethnicity", "freq:age:ethnicity")
anova.tables = rbind(abs.anova, mag.anova, mag.norm.anova, g.norm.anova, b.norm.anova, ear.canal.anova)

write.table(anova.tables, file = "anova.tables.txt", sep = ",", quote = FALSE, row.names = F)
...
# post hoc plots
b.norm.lsmean.age = lsmeans(b.norm.f.final, specs = "Frequency", by = "age.group", type = "response")
...
# plot ethnicity effect
b.norm.lsmean.eth = lsmeans(b.norm.f.final, specs = "Frequency", by = c("age.group", "maternal.ethnicity"), type = "response")
