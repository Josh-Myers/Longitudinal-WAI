...
ear.canal.lsmean.age = emmeans(ear.canal.f_log, specs = "age.group") 
...
abs.f_t = lmer(Absorbance_t ~ Frequency * age.group + gender + maternal.ethnicity + ear + (1 | id.res/ear.id), abs.2.long, REML = F)
abs.f.b.full = lmer_alt(Absorbance_pos ~ Frequency * age.group * maternal.ethnicity + gender + ear + (1 || id.res/ear.id), abs.2.long, REML = FALSE)

summary(abs.f.b.full)
# remove variables one at a time to see if significant

# test if main effect are signficant
abs.f.freq = lmer_alt(Absorbance_pos ~ age.group * maternal.ethnicity + gender + ear + (1 | id.res/ear.id), 
...
abs.f.age = lmer_alt(Absorbance_pos ~ Frequency * maternal.ethnicity + gender + ear + (1 | id.res/ear.id), 
                      abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.age.anova = anova(abs.f.age, abs.f.b.full)
abs.age.anova = abs.age.anova[2,6:8]

abs.f.eth = lmer_alt(Absorbance_pos ~ Frequency * age.group + gender + ear + (1 | id.res/ear.id), 
                      abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
abs.eth.anova = anova(abs.f.eth, abs.f.b.full)
abs.eth.anova = abs.eth.anova[2,6:8]

abs.f.b.ear = lmer_alt(Absorbance_pos ~ Frequency * age.group * maternal.ethnicity + gender + (1 | id.res/ear.id), 
                  abs.2.long, family = beta_family(link = "logit"), REML = FALSE)
...
# logit transform the response because proportion https://stats.stackexchange.com/questions/233366/how-to-fit-a-mixed-model-with-response-variable-between-0-and-1
logit_t = log(abs.2.long$Absorbance_pos / (1 - abs.2.long$Absorbance_pos))
abs.2.long$logit_t = logit_t

abs.f.b.full = lmer_alt(logit_t ~ Frequency * age.group * maternal.ethnicity + gender + ear + (1 | id.res/ear.id), abs.2.long, 
                        REML = FALSE)
anova(abs.f.b.full)
Anova(abs.f.b.full)
summary(abs.f.b.full)
...
### G and B at the tip - not using - just doing mag at the tip (G and B just normalized models)
## g
